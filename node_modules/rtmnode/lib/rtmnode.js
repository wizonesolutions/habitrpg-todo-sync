...
function RtmNode(apiKey, sharedSecret) {
  ...
  this.getFrob = function(existingFrob, callback) {
    if (!existingFrob) {
      ...
      console.log("Pre-getSig " + util.inspect(frobParams));
      frobParams.api_sig = this.getSignature(frobParams);

      console.log(util.inspect(frobParams));
  ...
  this.getToken = function(frob, callback) {
    ...
    console.log('this.defaultParameters getToken: ' + util.inspect(this.defaultParameters));
    tokenParams.method = "rtm.auth.getToken";
    tokenParams.frob = frob;

    console.log(util.inspect(tokenParams));
  ...
  this.checkToken = function(authToken, callback) {
    ...
    console.log(util.inspect(checkParams));
    ...
    request.get(apiRequest)
      ...
      .end(function(res) {
        if (res.ok) {
          ...
          console.log('Result from authToken: ' + util.inspect(res.text, {showHidden: true}));
          if (callback) {
  ...
  this.getTasks = function(listId, filter, lastSync, callback) {
    ...
    console.log(util.inspect(taskParams));
    ...
    request.get(apiRequest)
      ...
      .end(function(res) {
        ...
        if (res.ok && res.text.rsp.stat == "ok") {
          // Failing is handled down the chain, so we don't have to check for "ok" in res.text.rsp.stat
          console.log('Result from getTasks (probably big): ' + util.inspect(res.text, {showHidden: true}));
          if (callback) {
